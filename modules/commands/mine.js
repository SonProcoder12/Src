...
module.exports.config = {
	...
	switch (s.type) {
		case "shop":
			switch (a.body) {
				...
				case "3": {
					...
					if ("repair" == n.data.msg) return e.sendMessage("[HỆ THỐNG MINE] - Cúp Của Bạn Đã Hết Độ Bền ! Vui Lòng Sửa Chữa!", a.threadID);
					if ("cd" == n.data.msg) return e.sendMessage("[ HỆ THỐNG MINE ] - Vui Lòng Chờ!", a.threadID);
					if ("chuadangky" == n.data.msg) return e.sendMessage("[HỆ THỐNG MINE] - Bạn Chưa Đăng Kí Hệ Thống", a.threadID); {
				...
				case "4":
					if (i < 1000) e.sendMessage("[ HỆ THỐNG MINE ] - Bạn Không Đủ Coin Để Nâng Cấp", a.threadID);
					else {
						const n = await axios.get(`https://project-mine.hbaoluvhue.repl.co/upgrade?uid=${a.senderID}`, a.threadID);
						if ("false" == n.data.msg) e.sendMessage("[ HỆ THỐNG MINE ] - Bạn Chưa Mua Cúp! Không Thể Phù Phép", a.threadID);
						else if ("max" == n.data.msg) e.sendMessage("[ HỆ THỐNG MINE ] - Cúp Của Bạn Đã Được Phù Phép Tối Đa", a.threadID);
						else {
				...
				case "7": {
					...
					if ("chuadangky" == s.data.msg) return e.sendMessage("[ HỆ THỐNG MINE ] - Bạn Chưa Đăng Kí Hệ Thống", a.threadID); {
						const r = s.data.damage,
							d = s.data.hp,
							c = s.data.msg;
						if (s.data.images, "die" == s.data.msg) return i < 2000 ? e.sendMessage("[ HỆ THỐNG DUNGEON ] - Số Dư Của Bạn Không Đủ Đề Hồi Sinh ! Vui Lòng Kiếm Đủ 2000 Coin Để Được Hồi Sinh", a.threadID) : (await t.decreaseMoney(a.senderID, 2000), e.sendMessage("[HỆ THỐNG DUNGEON] - Bạn đã chết ! Hệ Thống Sẽ Tự Động Hồi Sinh Với Cái Gía Là 2000$", a.threadID));
						if ("repair" == s.data.msg) return e.sendMessage("[HỆ THỐNG DUNGEON] - Kiếm Của Bạn Đã Hết Độ Bền ! Vui Lòng Sửa Chữa Lại", a.threadID);
		...
		case "getpickaxe":
			if (a.body == a.body) {
				if (i < c) return e.sendMessage("[ HỆ THỐNG MINE ] - Bạn Không Đủ Coin Để Mua Cúp", a.threadID); {
					const n = await axios.get(`https://project-mine.hbaoluvhue.repl.co/shoppickaxe?buy=${a.body}&uid=${a.senderID}`);
					return "false" == n.data.data ? e.sendMessage("[ HỆ THỐNG MINE ] - Bạn Chưa Đăng Kí Hệ Thống", a.threadID) : "max" == n.data.msg ? e.sendMessage("[ HỆ THỐNG MINE ] - Bạn chỉ có thể mua 1 cúp", a.threadID) : (await t.decreaseMoney(a.senderID, c), p = () => e.sendMessage({
						body: `${n.data.msg}`,
		...
		case "repair":
			if (a.body == a.body) {
				if (i < 2000) return e.sendMessage("[ HỆ THỐNG MINE ]Bạn Không Đủ Coin Để Phù Phép\nSố Tiền : 2000$", a.threadID); {
		...
		case "weapons":
			if (a.body == a.body) {
				if (i < c) return e.sendMessage("[ HỆ THỐNG MINE ] - Bạn Không Đủ Coin Để Mua Cúp", a.threadID); {
					...
					if ("max" == n.data.msg) e.sendMessage("Tối Đa Chỉ Mua Được 1 Kiếm\nBạn Có Thể Phù Phép Kiếm", a.threadID);
					else {
						...
						e.sendMessage("[ HỆ THỐNG MINE ] - Bạn Chưa Đăng Kí Hệ Thống", a.threadID)
					}
				}
			}
	}
